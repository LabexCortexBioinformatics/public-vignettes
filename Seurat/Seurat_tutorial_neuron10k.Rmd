...
neur <- CreateSeuratObject(counts = seurat_matrix, project = "neuron_1k", min.cells = 3, min.features = 100)
neur
```
...
neur$percent.mt <- PercentageFeatureSet(seur, pattern = "^mt-")
neur$percent.ribo <- PercentageFeatureSet(seur, pattern = "^Rp[sl][[:digit:]]")
VlnPlot(neur, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.ribo"), ncol = 4, pt.size = 0.1)
```

Distribution of the data
```{r, fig.width=6, fig.asp=0.6, warning=FALSE, error=FALSE}
FeatureScatter(neur, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
```

We remove cells that makes nCount_RNA plot unreadable, and cells which have too much mitochondrial genes.
```{r}
neur = subset(neur, subset = (percent.mt < 20 & nCount_RNA < 50000))
```
...
```{r}
neur = subset(neur, subset = (nFeature_RNA > 1000 & nCount_RNA < 30000) & percent.mt < 15)
```
...
```{r}
neur <- SCTransform(neur, method = "glmGamPoi", verbose = FALSE)
```
...
### Store Seurat object
```{r}
saveRDS(seur, "/mnt/Data1/public_vignettes/seurat_data/neuron_10k.rds")
```
